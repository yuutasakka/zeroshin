-- ========================================
-- Zeroshin Admin - Fixed Database Schema
-- データ型不整合修正版スキーマ
-- ========================================
...
CREATE TABLE IF NOT EXISTS admin_registrations (
    ...
    approved_by UUID REFERENCES admin_credentials(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_admin_registrations_approved_by ON admin_registrations(approved_by);
...
CREATE TABLE IF NOT EXISTS admin_approval_history (
    ...
    performed_by UUID REFERENCES admin_credentials(id) ON DELETE SET NULL,
    comment TEXT,
    metadata JSONB,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT now()
);
...
CREATE INDEX IF NOT EXISTS idx_admin_approval_history_performed_by ON admin_approval_history(performed_by);
...
CREATE INDEX IF NOT EXISTS idx_pending_admin_approvals_approved_by ON pending_admin_approvals(approved_by);
...
-- 管理者IDでユーザー名を取得
CREATE OR REPLACE FUNCTION get_admin_username(admin_uuid UUID)
RETURNS VARCHAR(255) AS $$
DECLARE
    username VARCHAR(255);
BEGIN
    SELECT ac.username INTO username
    FROM admin_credentials ac
    WHERE ac.id = admin_uuid AND ac.is_active = true;
...
-- ユーザー名で管理者IDを取得
CREATE OR REPLACE FUNCTION get_admin_id_by_username(username_input VARCHAR(255))
RETURNS UUID AS $$
DECLARE
    admin_uuid UUID;
BEGIN
    SELECT ac.id INTO admin_uuid
    FROM admin_credentials ac
    WHERE ac.username = username_input AND ac.is_active = true;
...
BEGIN
    RAISE NOTICE '========================================';
    RAISE NOTICE 'Zeroshin Admin Database Schema (Fixed) Created Successfully!';
    RAISE NOTICE '========================================';
    RAISE NOTICE 'Key Changes:';
    RAISE NOTICE '- All primary keys unified to UUID';
    RAISE NOTICE '- Foreign key constraints fixed';
    RAISE NOTICE '- Enhanced referential integrity';
    RAISE NOTICE '- Helper functions for UUID management';
    RAISE NOTICE '========================================';
    RAISE NOTICE 'Next Steps:';
    RAISE NOTICE '1. Run RLS policies: rls_policies_fixed.sql';
    RAISE NOTICE '2. Create initial admin user: create_initial_admin_fixed.sql';
