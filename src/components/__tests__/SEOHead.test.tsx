...
describe('SEOHead', () => {
  ...
  test('デフォルトのメタタグが正しく設定される', () => {
    ...
    // タイトル
    expect(titleTag.props.children).toBe('MoneyTicket - お金診断アプリ');
  ...
  test('Open Graphタグが正しく設定される', () => {
    ...
    expect(ogTitle.props.content).toBe('MoneyTicket - お金診断アプリ');
    ...
    expect(ogSiteName.props.content).toBe('MoneyTicket');
  });
  ...
  test('構造化データが正しく設定される', () => {
    ...
    expect(orgSchema.name).toBe('MoneyTicket');
