...
const EnhancedHero: React.FC<EnhancedHeroProps> = ({ onStartDiagnosis }) => {
  ...
  return (
    <>
      <section className="relative min-h-[70vh] flex items-center justify-center overflow-hidden bg-gradient-to-br from-gray-50 via-white to-gray-100 pt-16 md:pt-0">
        {/* 背景パターン */}
      ...
      {/* アニメーション円 */}
      <div className="absolute top-20 right-10 w-64 h-64 bg-gradient-to-br from-blue-400 to-blue-600 rounded-full blur-3xl opacity-20 animate-pulse" />
      <div className="absolute bottom-20 left-10 w-48 h-48 bg-gradient-to-br from-orange-400 to-orange-600 rounded-full blur-3xl opacity-20 animate-pulse" style={{ animationDelay: '1s' }} />

      <div className="relative z-10 max-w-5xl mx-auto px-4 text-center">
        {/* バッジ - スマホ対応で位置調整 */}
        <div className={`inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-full text-sm font-medium mb-4 md:mb-6 transform transition-all duration-1000 ${isAnimated ? 'translate-y-0 opacity-100' : '-translate-y-4 opacity-0'}`}>
        ...
        <div className={`mb-8 md:mb-12 transform transition-all duration-1000 delay-400 ${isAnimated ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
          <button
            ...
            className="group relative inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-orange-600 to-orange-700 text-white font-bold text-lg rounded-full shadow-xl hover:shadow-2xl transform hover:-translate-y-1 transition-all duration-300"
          >
          ...
          {stats.map((stat, index) => (
            ...
            >
              <div className={`text-lg md:text-2xl font-bold transition-colors duration-500 ${
                currentStat === index 
                  ? 'text-transparent bg-gradient-to-r from-orange-600 to-orange-700 bg-clip-text' 
                  : 'text-gray-700'
              }`}>
                {stat.number}
              </div>
